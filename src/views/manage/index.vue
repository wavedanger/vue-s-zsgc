<template>
    <div class="manage tc">
         <div class="input-area">
            <input type="text" placeholder="请输入要搜索的名字" v-model="searchValue">
            <button @click="add">新增</button>
        </div>
        <div class="input-area" v-show="showAdd">
            <input type="text" placeholder="请输入新名字" v-model="nameValue">
        ...
        <table>
            ...
            <tr v-for="(item,index) in people">
                <td>{{item.name}}</td>
                <td v-bind:id="index"><span v-on:click="editName">编辑</span><span v-on:click="delName">删除</span></td>
            </tr>
        </table>
        <div class="wrap" v-show="showEdit">
            <div class="content">
                ...
                <button v-on:click="sure">确定</button>
            </div>
        </div>
...
<style>
	...
	.manage>button{width:100px; height:40px; line-height:40px; background-color:#41b883; border: none; border-radius:5px; font-size:16px; color:#fff;}
    ...
    computed:{
        people:function(){
            ...
            for(var i=0;i<this.peoples.length;i++){
                if(this.peoples[i].name.search(this.searchValue)!=-1){
